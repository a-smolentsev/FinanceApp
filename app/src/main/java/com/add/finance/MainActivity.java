...
//Отправляем запрос в БД
        ...
        Cursor cursor = mDb.rawQuery("SELECT * FROM " + DBHelper.add_summ + " ORDER BY strftime('%d-%m-%Y'," + DBHelper.date1 + " ) ASC", null);
...
//Создаем адаптер
        ...
        int mainSum; //сумма вся Доход - расход

        mainSum=summs()-rashod();
        summa.setText(Integer.toString(mainSum));
               // summa.setText(Integer.toString(mainSum)
        return;
    }
        ...
        public void onClick2 (View view){
            Intent intent = new Intent(MainActivity.this, add_rashod.class);
            startActivity(intent);

        }
        public void onClick3 (View view){
            Intent intent = new Intent(MainActivity.this, rashod_list.class);
            startActivity(intent);
    ...
    public int summs() {
        ...
        String sumQuery=String.format("SELECT SUM(%s) as Total FROM %s",mDBHelper.summ1,mDBHelper.add_summ);
    ...
    public int rashod() {
        int sum=0;
        mDb = mDBHelper.getReadableDatabase();
        String sumQuery=String.format("SELECT SUM(%s) as Total FROM %s",mDBHelper.summ2,mDBHelper.table2);
        Cursor cursor=mDb.rawQuery(sumQuery,null);
        if(cursor.moveToFirst())
            sum= cursor.getInt(0);
        return sum;
